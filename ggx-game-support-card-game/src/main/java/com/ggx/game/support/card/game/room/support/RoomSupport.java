...
extends ExecutorSupport, SendMessageSupport {
	 ...
	 * 2019-06-04 13:52:25
	 */
	default List<P> getInGamePlayers(CheckCondition<P> condition) {
	 ...
	 * 2019-07-11 09:56:53
	 */
	default P getInGamePlayer(CheckCondition<P> condition) {
		for (Entry<Object, P> entry : getPlayers().entrySet()) {
			P player = entry.getValue();
	 ...
	 * 2019-02-23 16:49:17
	 */
	default void bcToAllPlayer(Message message, CheckCondition<P> condition) {
		eachPlayer((player) -> {
	 ...
	 * @return
	 */
	default P getNextPlayer(int startSeatNo, CheckCondition<P> condition) {
			//获取排序后的玩家
			List<P> pList = getSortedInGamePlayerList();
			//排除起始座位号
			if (condition != null) {
				pList = pList.stream().filter(o -> o.getIndex() != startSeatNo && condition.check(o)).collect(Collectors.toList());
			}else {
				pList = pList.stream().filter(o -> o.getIndex() != startSeatNo).collect(Collectors.toList());
			}
	...
	default List<P> getSortedInGamePlayerList() {
		...
		Collections.sort(inGamePlayers, (x, y) -> x.getIndex() - y.getIndex());
	 ...
	 * 2019-05-21 15:54:04
	 */
	default int countInGamePlayers(CheckCondition<P> condition) {
		int i = 0;
		P player = null;
		for (Entry<Object, P> entry : getPlayers().entrySet()) {
